...
param(
    ...
    [Parameter(Mandatory = $false)]
    [string]$LyncFQDN = "gmzrzapp061.ferchau.local", ### "sfb2019.ferchau.local",
    [Parameter(Mandatory = $false)]
...
$query = "SELECT " + $column['id'] + "," + $column['newvalue'] + "," + $column['attribute'] + "," + $column['userDN'] + "," + $column['emailtext'] + "," + $column['emailrecipient'] + "," + $column['emailsubject'] + " FROM $SQLTable WHERE [AD_Attribut] not in ('Group','RemoveGroup','Entfernen','Deaktivieren') and verarbeitet IS NULL"
